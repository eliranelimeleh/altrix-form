...
ffmpeg -loglevel error -y -i "${OUT}_hq.mp4" -i /tmp/_seg.mp4 -filter_complex \
 "[0:v]trim=end_frame=$A,setpts=PTS-STARTPTS[a];[0:v]trim=start_frame=$((B+1)),setpts=PTS-STARTPTS[c];[1:v]setpts=PTS-STARTPTS[b];[a][b][c]concat=n=3:v=1:a=0,fps=30[v]" \
 -map "[v]" -map 0:a -r 30 -c:v libx264 -preset veryfast -crf 17 -pix_fmt yuv420p -c:a copy -movflags +faststart /tmp/_patched.mp4
mv /tmp/_patched.mp4 "${OUT}_hq.mp4"
# size the video bitrate so the delivery file lands at ~28MB (chat upload limit is 30MB)
DUR=$( (ffmpeg -i "${OUT}_hq.mp4" 2>&1 || true) | grep -oE "Duration: [0-9:.]+" | awk -F'[: ]' '{print $3*3600+$4*60+$5}')
